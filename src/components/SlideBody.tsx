...
export class SlideBody extends Rect {
  private imageRef = createRef<JinImage>();
  ...
  public *playSlides(): ThreadGenerator {
    ...
    for (let i = 0; i < this.slides.length; i++) {
      ...
      if (this.slides[i].animations?.image) {
        ...
        yield* this.imageRef().playAnimation(this.slides[i].animations.image);
      } else {
        // Default animation if none specified - run for exact slide duration
        yield* this.imageRef().startAnimation();
      }
   ...
   * 이미지 참조를 가져오는 메서드
   */
  public getImageRef(): Reference<JinImage> {
 ...
 * (기존 코드와의 호환성을 위해 유지)
 */
export function createSlideBody(props: SlideBodyProps): { imageRef: Reference<JinImage>; playSlides: () => ThreadGenerator } {
